...
GRAVITY_VEC = torch.tensor([[0.0, 0.0, -1.0]]).to('cuda')
GAIT_PATTERN = torch.tensor([0, torch.pi, torch.pi, 0])


def phase_obs(env: ManagerBasedRLEnv) -> torch.Tensor:
    phase_tensor = torch.cat([torch.sin(env.ftg.phase), torch.cos(env.ftg.phase)], dim=-1)
    return phase_tensor


def foot_phase_obs(env: ManagerBasedRLEnv) -> torch.Tensor:
    phase_tensor = torch.cat([torch.sin(env.ftg.foot_phase), torch.cos(env.ftg.foot_phase)], dim=-1)
    return phase_tensor
